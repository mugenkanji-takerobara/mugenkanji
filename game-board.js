...
(() => {
  ...
  app.着地後処理 = function () {
    ...
    // 上端のどこかに漢字錠があるだけではゲームオーバーにしない。
    // 次の漢字錠が実際の出現位置へ置けない時だけ、新規ピース側で終了判定する。
    app.次の特の符確認();
  };
  ...
  app.ボーナス終了 = function () {
    ...
    app.ボーナス確認閉じる?.();
    app.特の符確認閉じる?.();
    // 1枚目の「岳」「代」を使い終えた時点で、残っている保存分を必ず再有効化する。
    app.特の符表示更新?.();
    app.ゲーム状態保存?.();
  ...
  app.岳消去確定 = function () {
    ...
    状態.得点 += 加点;
    app.消去音再生?.(2);
    app.技結果表示?.("山岳鎮定", `${対象}を${消去数}個消去　＋${加点}点`);
  ...
  app.代入替確定 = function () {
    ...
    if (状態.ボーナス残数 <= 0) {
      app.技結果表示?.("代身遷宮", "入れ替え完了");
      app.ボーナス終了();
    } else {
      app.技結果表示?.("代身遷宮", `入れ替え完了　あと${状態.ボーナス残数}回`);
      app.描画?.();
    }
  };
})();
